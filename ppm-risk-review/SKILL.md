...
## Operational caution — confidentiality / NDA

Subscription agreements and many PPMs contain confidentiality / NDA clauses that restrict sharing the documents with third parties. Because this skill ingests those documents, be mindful that processing them through third-party tools — cloud LLM APIs, document-analysis services, consumer chatbots — can implicate those clauses. When ingesting a PPM:

- Prefer runtimes that process locally or under a **no-retention / no-training** policy.
- If the runtime sends document content to a third-party API, confirm the subscription agreement's confidentiality clause permits it — many do not.
- Never paste PPM excerpts into public / free-tier AI tools that may train on inputs.
- When in doubt, redact PII and proprietary portfolio detail before any external processing step.

This is operational guidance about how the skill handles documents — not a deal finding, so it does not go on the memo's page-one list.
...
2. **Extract text.** Use `scripts/extract_pdf.sh <pdf> <txt>` to dump the PDF — it wraps `pdftotext -layout` (poppler), auto-installs poppler when it can, and falls back to a pure-Python `pypdf` extraction if poppler is unavailable. PPMs are 40–200 pages of dense text and reading the PDF visually is too slow; the layout-preserved text is sufficient for nearly all analysis (the `pypdf` fallback loses layout but is fine for most prose sections).
...
4. **Run the rubric.** Walk the document against the rubric in `references/rubric.md` and capture findings against each bucket, reading each through the rubric's three **analytical lenses** — fund counsel, prospective investor, and lender-to-the-fund (the credit lens applies to every fund, not just notes). Always look for what is **missing** as well as what is present — missing fee tables, missing track record, empty exhibits, and missing GP commitment are the highest-signal risks. Where a transcript exists, use it to corroborate findings, capture sponsor on-the-record admissions, and surface new questions — never to override the documents.
   - **4.5. Run the Risk Radar — the variant that matches the underlying asset (rubric §6.5–6.7).** **§6.5 (equity / pro forma)** for RE equity / value-add / development — plot each pro-forma assumption against its ~50-year historical range (cap-rate / expense / vacancy / deliveries / absorption), pulling the series from tools/web, **not** the deck. **§6.6 (credit)** for debt funds and note programs — plot the book and the manager against the credit-loss mechanism (default × workout × collateral × forced-seller; coupon-funding integrity), watching the two kill shots (coupon-funded-from-proceeds; undisclosed subordination). **§6.7 (land)** for land with no stabilized cash flow — plot basis / carry / entitlement / terminal-liquidity, watching the kill shot (a pref/distribution on a no-income asset). In every variant: flag each outer-ring spoke, tag it by zone (`[LOCKED]`/`[SPONSOR]`/`[MARKET]`), escalate one tier if 2+ sit at the outer edge (a single kill-shot spoke is itself page-one/FATAL), and run the stacked-stress scenario. Complements (does not replace) the step-6 current-market benchmarking. A fund that entitles then builds runs §6.7 then §6.5; a land-collateralized note runs §6.6 with §6.7's asset spokes.
...
When a Risk Radar stress test was run (§6.5/§6.6/§6.7), add a **"Knowingly Accepted Risks"** section after the bucketed findings and before the Uncertainty Map: list the outer-ring spokes the IC is accepting with eyes open, each with the accept-rationale, tagged by zone (e.g., "`[MARKET]` exit cap assumed at 4.5% — outer ring; accepted because the 5-year hold gives cap-rate optionality and we will exit early if the cycle turns"). This is distinct from `[STRENGTH]` (a point in the deal's favor) and from the Uncertainty Map (what still needs clarifying) — it is "we know this is a risk and are taking it anyway." A visible risk is not automatically disqualifying; the point is whether it is a risk the IC is willing to accept.
...
- `scripts/extract_pdf.sh <pdf> <txt>` — text extraction. Prefers `pdftotext -layout` (poppler-utils; layout-preserved, best for PPMs and already installed in the Cowork sandbox), auto-installs poppler via `brew`/`apt`/`dnf` when possible, and falls back to a pure-Python `pypdf` extraction (auto-installed via `pip`; no layout) so it works off the sandbox. Raw `pdftotext -layout <pdf> <txt>` is fine directly when poppler is present.
